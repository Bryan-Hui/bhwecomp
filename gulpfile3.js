...
gulp.task(`${id}-wxs`, () => {
    return gulp.src('**/*.wxs', { cwd: 'src', base: 'src' })
        ...
        .pipe(gulp.dest('dist')); // output all wxs files to the dist directory
});
...
gulp.task('dev', gulp.series(`${id}-clean`, `${id}-example`, `${id}-wxs`, `${id}-wxml`, `${id}-less`, `${id}-ts`, `${id}-json`, gulp.parallel(`${id}-watch-wxml`, `${id}-watch-less`, `${id}-watch-ts`, `${id}-watch-json`, `${id}-watch-example`)))